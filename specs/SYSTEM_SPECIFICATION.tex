...
\affil[1]{KDSZ Investments Ltd, London, UK\\
\texttt{kim@kdszinvestments.com}}
...
The safety verification of neural networks is a central challenge in AI safety research.
For monolithic architectures---including transformers, large language models, and jointly-trained mixture-of-experts systems---formal reasoning about properties such as knowledge attribution, guaranteed forgetting, and compositional stability remains intractable in practice.
The fundamental difficulty is that knowledge in these systems is distributed across entangled parameters, making it impossible to localise influence, bound contributions, or guarantee removal of specific knowledge.

We study an architecture that introduces structural constraints designed to make such reasoning tractable.
The system consists of a pool of independently trained expert modules, each frozen after creation, composed at inference time through sparse top-K routing in a shared representation space.
...
\begin{remark}
In standard MoE (Switch Transformer, GShard, Mixtral), experts share gradients during training, weights are mutable, the pool is fixed at training time, and knowledge is distributed across all experts. These design choices make the same formal questions intractable for the same reasons they are intractable in monolithic models.
\end{remark}
...
\noindent\textbf{Proof strategy.} Since existing modules are frozen and only the router changes, this becomes a question about the sample complexity of learning a routing policy over $N+1$ options versus $N$ options. PAC-Bayes bounds or Rademacher complexity arguments for the router class may apply.
...
Can the sparse routing mechanism be shown to satisfy $(\epsilon, \delta)$-differential privacy with respect to the presence or absence of a specific module in the pool? If so, what are the privacy parameters as a function of $K$, $N$, and the router architecture?
\end{conjecture}

\noindent\textbf{Proof strategy.} The sparsity of routing ($K \ll N$) limits the influence of any single module. This may connect to the literature on differential privacy in mechanism design, where the ``mechanism'' is the router and the ``database'' is the module pool.
...
\textbf{Mixture-of-Experts.} Switch Transformer~\cite{fedus2022switch}, GShard~\cite{lepikhin2021gshard}, and Mixtral~\cite{jiang2024mixtral} train experts jointly with shared gradients, producing entangled knowledge representations. Our independent training and permanent freezing constraints create a structurally different system.

\textbf{Modular Networks.} Neural module networks~\cite{andreas2016neural} compose specialised modules for visual question answering, but modules are jointly trained and task-specific. Our modules are domain-general frozen functions.
...
\textbf{Formal Verification of Neural Networks.} Existing work focuses on input-output properties of fixed networks (e.g., robustness verification). Our questions concern compositional properties of modular systems---a different class of formal problem.
...
The research programme requires formal methods expertise---specifically in PAC learning theory, information theory, differential privacy, and formal verification---to translate empirical observations into rigorous results.
...
% ============================================================
% REFERENCES (minimal, arXiv-friendly)
% ============================================================
